...
def data_preprocessing(data,target_columns):
    ...
    # data['요약문_한글키워드'] = data.apply(lambda x : ','.join(nouns(x['과제명'])) if x['요약문_한글키워드'] == 'NONE' else x['요약문_한글키워드'], axis = 1)
...
def ngram_vectorize(train_data, label, test_data, top_k) : 
    ...
    selector = SelectKBest(f_classif, k=min(25000,top_k))
...
def vectorize_data(train, test, top_ks, target_columns):
    ...
    for top_k, column in zip(top_ks, target_columns):
        train_input, test_input = ngram_vectorize(train[column], train['label'], test[column], min(5000,top_k))
...
for train_idx, valid_idx in skf.split(train_inputs[0], labels):
    ...
    ckpt_path = f'./00_2_dacon_climate/_MCP/{now}.ckpt'
...
sample = pd.read_csv('./00_2_dacon_climate/_data/sample_submission.csv')
sample['label'] = np.argmax(np.mean(predictions,axis=0), axis = 1)
sample.to_csv('./00_2_dacon_climate/_save/bert_baseline8.csv',index=False)